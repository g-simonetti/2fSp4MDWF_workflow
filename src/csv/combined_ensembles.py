...
def infer_use_name(fit_json_path):
    stem = Path(fit_json_path).stem
    marker = "_fits_"
    if marker in stem:
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Export a CSV collecting mres(N5) fit summaries and spectrum "
    ...
    parser.add_argument("--output_mres_csv", required=True)
    parser.add_argument("--output_finite_volume_csv", required=True)
    parser.add_argument("--output_chipt_csv", required=True)
    args = parser.parse_args()

    metadata_rows, metadata_lookup = load_metadata(args.metadata_csv)
    mres_rows = build_mres_rows(
        args.mres_fit_json,
        metadata_lookup,
        infer_use_name(args.mres_fit_json),
    )
    chipt_rows = []
    chipt_rows.extend(build_spectrum_rows(args.mv_fit_json, metadata_rows))
    chipt_rows.extend(build_spectrum_rows(args.fps_fit_json, metadata_rows))
    finite_volume_rows = build_finite_volume_rows(
